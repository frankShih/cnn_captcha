...
class CNN(object):
    def __init__(self, image_height, image_width, max_captcha, char_set, model_save_dir):
        ...
        self.model_save_dir = model_save_dir # model path
    ...
    def convert2gray(img):
        """
        The picture is converted to grayscale, if it is a 3-channel picture, it will be calculated, and the single-channel picture will be returned directly
        :param img:
        :return:
        """
        if len(img.shape)> 2:
    ...
    def text2vec(self, text):
        """
        Convert tags to oneHot encoding
        :param text: str
        :return: numpy.array
        """
        text_len = len(text)
        if text_len> self.max_captcha:
            raise ValueError('The longest verification code is {} characters'.format(self.max_captcha))
